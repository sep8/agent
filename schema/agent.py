...
class Plan(BaseModel):
    ...
    def format(self) -> Any:
        return "\n".join([f"{i+1}. {s.value}" for i, s in enumerate(self.steps)])
...
class BaseTool(BaseModel):
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        if (verbose):
            print("tool inputs: ", tool_input)
        
        response = self.func(tool_input, **kwargs)

        if (callbacks is not None):
            all_callbacks = [*callbacks, self.callback]
        else:
            all_callbacks = self.callback
